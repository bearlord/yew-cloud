...
use Yew\Plugins\Cloud\Gateway\Annotation\Controller;
use Yew\Plugins\Cloud\Gateway\Annotation\RequestMapping;
use Yew\Plugins\Cloud\Gateway\Aspect\RouteAspect;
use Yew\Plugins\Cloud\Gateway\Filter\FilterManager;
use Yew\Plugins\Cloud\Gateway\Filter\JsonResponseFilter;
use Yew\Plugins\Cloud\Gateway\Filter\ServerFilter;
        ...
        if (!empty($routeRole->getPortTypes())) {
            foreach ($routeRole->getPortTypes() as $portType) {
                foreach ($this->gatewayConfigs as $gatewayConfig) {
                    if ($gatewayConfig->getBaseType() == $portType) {
                        $couldPortNames[] = $gatewayConfig->getName();
                    }
                }
            }
        } else {
